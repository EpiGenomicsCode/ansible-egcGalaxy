...
if __name__ == '__main__':
    ...
    # get arguments
    try:
        optlist, alist = getopt.getopt(sys.argv[1:], 'hi:o:r:c:m:')
    ...
    print ("Row number (pixels):")
    print(row_num)
    print ("Col number (pixels):")
    print(col_num)

    if (method=="pillow"):
        from PIL import Image
        image = Image.open(input_file)
        new_image = image.resize((col_num, row_num))
        new_image.save(out_file)
    elif (method=="opencv"):
        import cv2
        img = cv2.imread(input_file)
        #img =  imread.imread(input_file)
        res = cv2.resize(img, dsize=(col_num, row_num), interpolation=cv2.INTER_AREA)
        #res = cv2.resize(img, dsize=(col_num, row_num), interpolation=cv2.INTER_AREA)
        cv2.imwrite(out_file, res)
    else:
        sys.stderr.write("(!)Invalid image compression method provided: %s\n" % method)
        raise ValueError